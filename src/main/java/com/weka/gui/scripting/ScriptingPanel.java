...
package com.weka.gui.scripting;
...
import com.weka.core.Tee;
import com.weka.gui.PropertyDialog;
import com.weka.gui.ReaderToTextPane;
import com.weka.gui.scripting.event.TitleUpdatedEvent;
import com.weka.gui.scripting.event.TitleUpdatedListener;
...
public abstract class ScriptingPanel
  ...
  /** The new com.output stream for System.out. */
  protected PipedOutputStream m_POO;

  /** The new com.output stream for System.err. */
  protected PipedOutputStream m_POE;

  /** The thread that sends com.output from m_POO to the com.output box. */
  protected ReaderToTextPane m_OutRedirector;

  /** The thread that sends com.output from m_POE to the com.output box. */
  protected ReaderToTextPane m_ErrRedirector;

  /** The Tee that the com.output stream will be part of. We need a reference to
   * it at the class level so that we can remove the stream from it when
   * the panel is terminated. */
  protected Tee m_Tee;

  /** The print stream that is wrapped around the piped com.output stream. We need a
  ...
  /** The print stream that is wrapped around the piped error com.output stream. We need a
   * a reference to it at the class level so that we can remove it from the tee
   * when the panel is terminated.
   */
  ...
  /**
   * Terminates this panel, i.e., terminates the com.output threads it started. Removes
   * the com.output and error streams from the corresponding tees as well so that garbage collection succeeds.
   */
  ...
  /**
   * Returns the text area that is used for displaying com.output on stdout
   * and stderr.
   * 
