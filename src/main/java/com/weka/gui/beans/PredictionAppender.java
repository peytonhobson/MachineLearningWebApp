...
public class PredictionAppender extends JPanel implements DataSource,
  ...
  protected transient com.weka.gui.Logger m_logger;
  ...
  public void acceptClassifier(IncrementalClassifierEvent e) {
    com.weka.classifiers.Classifier classifier = e.getClassifier();
          ...
          if (m_dataSourceListeners.size() > 0 || m_testSetListeners.size() > 0) {
            ...
            for (int i = 0; i < testSet.numInstances(); i++) {
              ...
              if (tempInst.isMissing(tempInst.classIndex())
                && !(classifier instanceof com.weka.classifiers.misc.InputMappedClassifier)) {
  ...
  public void acceptClusterer(BatchClustererEvent e) {
    if (m_dataSourceListeners.size() > 0 || m_trainingSetListeners.size() > 0
      ...
      com.weka.clusterers.Clusterer clusterer = e.getClusterer();
      String test;
  ...
  private Instances makeDataSetProbabilities(Instances insts, Instances format,
                                             com.weka.classifiers.Classifier classifier, String relationNameModifier)
    throws Exception {
    ...
    for (int i = 0; i < format.classAttribute().numValues(); i++) {
      com.weka.filters.unsupervised.attribute.Add addF =
        new com.weka.filters.unsupervised.attribute.Add();
      ...
      newInstances = com.weka.filters.Filter.useFilter(newInstances, addF);
    }
  ...
  private Instances makeDataSetClass(Instances insts, Instances structure,
    ...
    Instances newInstances = com.weka.filters.Filter.useFilter(insts, addF);
    newInstances.setRelationName(insts.relationName() + relationNameModifier);
    return newInstances;
  }

  private Instances makeClusterDataSetProbabilities(Instances format,
                                                    com.weka.clusterers.Clusterer clusterer, String relationNameModifier)
    throws Exception {
  ...
  private Instances makeClusterDataSetClass(Instances format,
    ...
    Instances newInstances = com.weka.filters.Filter.useFilter(format, addF);
    newInstances.setRelationName(format.relationName() + relationNameModifier);
    return newInstances;
  }
   ...
   */
  @Override
  public void setLog(com.weka.gui.Logger logger) {
    m_logger = logger;
  }
