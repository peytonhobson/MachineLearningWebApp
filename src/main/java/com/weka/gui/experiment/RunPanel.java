...
public class RunPanel
  ...
  /**
   * Creates the com.run panel with no initial experiment.
   */
  public RunPanel() {
      ...
      if (m_RunThread == null) {
        ...
        if (proceed) {
          try {
            ...
            ex.printStackTrace();
            logMessage("Problem creating experiment copy to com.run: "
                + ex.getMessage());
          }
        }
      }
  ...
  /**
   * Tests out the com.run panel from the command line.
   *
   * @param args may contain options specifying an experiment to com.run.
   */
