...
import com.weka.classifiers.pmml.consumer.NeuralNetwork.MiningFunction;
...
import com.weka.core.pmml.VectorDictionary;
import com.weka.core.pmml.VectorInstance;
...
//      System.err.println("Result: " + result);
  ...
  static class SupportVectorMachine implements Serializable {
     ...
     * If the com.output of the machine is < 0, then a 1 is entered into the array
...
/*      if (result < 0 && m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
      ...
      if (cMethod == classificationMethod.NONE || 
        ...
        // ----------------------------------------------------------------------
        // The PMML 3.2 spec states that the com.output of the machine should lie
        ...
        // non standard, and indeed, in the 4.0 spec, it has changed to com.output < threshold
        ...
        // which is PMML version 3.2, produces com.output between -1 and 1 (give or take).
        // Implementing the 3.2 scoring mechanism as described in the spec (and truncating 
        // com.output at 0 and 1) results in all the predicted labels getting flipped on the data
        ...
        if (cMethod == classificationMethod.ONE_AGAINST_ALL) {
          // smallest value com.output by a machine is the predicted class
          preds[targetIndex] = result;
        } else {
          // one-vs-one
