...
/**
 * Class that wraps a {@code com.weka.gui.Logger} and filters log messages according
 * to the set logging level. Note that warnings and errors reported via the
 * logWarning() and logError() methods will always be com.output regardless of the
 * logging level set.
 *
...
public class LogManager implements LogHandler {
  ...
  /** True if status messages should be com.output as well as log messages */
   ...
   * be com.output, regardless of the logging level set
   */
   ...
   * 
   * @param log the log to wrap
   * @param status true if warning and error messages should be com.output to the
