...
import com.weka.attributeSelection.RankedOutputSearch;
import com.weka.attributeSelection.Ranker;
import com.weka.attributeSelection.SubsetEvaluator;
import com.weka.attributeSelection.UnsupervisedAttributeEvaluator;
import com.weka.attributeSelection.UnsupervisedSubsetEvaluator;
 ...
 * an "info" connection. Will com.output both attribute selection results (via text
 ...
 * step can either com.output a cross-validation style summary over all the folds or
...
public class ASEvaluator extends WekaAlgorithmWrapper {
  ...
  /**
   * Whether to com.output separate evaluation results for each fold of a xval or
  ...
  public Class getWrappedAlgorithmClass() {
    return com.weka.attributeSelection.ASEvaluation.class;
  }
  ...
  public ASEvaluation getEvaluator() {
    return (com.weka.attributeSelection.ASEvaluation) getWrappedAlgorithm();
  }
  ...
  /**
   * Set whether to com.output separate results for each fold of a cross-validation,
   * rather than averaging over folds.
   *
   * @param treatSeparately true if each fold will have results com.output
   */
  ...
  /**
   * Get whether to com.output separate results for each fold of a cross-validation,
   * rather than averaging over folds.
   *
   * @return true if each fold will have results com.output
   */
  ...
  /**
   * Initialize at the start of a com.run
   * 
   ...
   *
   * @param text the text to com.output
   ...
   * cross-validation folds when results are com.output for each separately
   *
   ...
   *
   * @param connType the connection type to com.output on
  ...
  protected void applyFiltering(String connType, int[] selectedAtts,
    ...
    try {
      ...
      if (transformer != null) {
        ...
        reduced = com.weka.filters.Filter.useFilter(data, r);
      }
      if (!isStopRequested()) {
  ...
  /**
   * Get a list of com.output connections that can be produced given the current
   * state of the step
   *
   * @return a list of com.output connections
   */
  @Override
  ...
  public String getCustomEditorForStep() {
    return "com.weka.gui.knowledgeflow.steps.ASEvaluatorStepEditorDialog";
  }
}
