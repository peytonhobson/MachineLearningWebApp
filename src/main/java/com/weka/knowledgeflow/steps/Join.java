...
public class Join extends BaseStep {
  ...
  protected synchronized Instance processBuffers() {
    if (m_firstBuffer.size() > 0 && m_secondBuffer.size() > 0) {
      ...
      if (cmp == 0) {
        // match on all keys - com.output joined instance
        Instance newInst =
  ...
  /**
   * Generate the header of the com.output instance structure
   */
  ...
  public List<String> getOutgoingConnectionTypes() {
    if (getStepManager().numIncomingConnections() > 0) {
      // we com.output the same connection type as the inputs
      List<String> result = new ArrayList<String>();
      result.addAll(getStepManager().getIncomingConnections().keySet());
      return result;
    }
  ...
  public String getCustomEditorForStep() {
    return "com.weka.gui.knowledgeflow.steps.JoinStepEditorDialog";
  }
}
