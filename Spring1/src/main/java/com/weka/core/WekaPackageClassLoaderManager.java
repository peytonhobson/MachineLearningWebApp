 ...
 * for each package. {@code Utils.forName()} and {@code com.weka.Run} use this
...
public class WekaPackageClassLoaderManager {
  ...
  /** Path to the com.weka.jar file on the classpath */
  ...
  /**
   * Return the path to the com.weka.jar file (if found) on the classpath.
   *
   * @return the path to the com.weka.jar file on the classpath, or null if no
   *         com.weka.jar file was found.
   */
   ...
   * the core com.weka classes) as an array of URLs. This is primarily used by
   ...
   * handle the situation where the com.weka.jar is loaded by a custom
   * (non-URLClassLoader) withn an app server (for example).
   * 
   * @return an array of URLs containing the entries available to the
   *         classloader that loads the core com.weka classes
   */
  public URL[] getWekaClassloaderClasspathEntries() {
    ...
    if (parent instanceof URLClassLoader) {
      ...
      // scan for com.weka.jar
      for (URL u : result) {
        if (u.toString().endsWith("com.weka.jar")) {
          try {
    ...
    if (ClassLoader.getSystemClassLoader().equals(getClass().getClassLoader())) {
      // we can process the java.class.path property for com.weka core stuff
   ...
   * java.class.path property. This will contain the com.weka.jar (or directory
   * containing com.weka classes) if Weka is launched as an application. It won't
   * contain Weka if com.weka is loaded by a child classloader (in an app server or
   ...
   * location of the com.weka.jar file on the file system.
   *
  ...
  protected static void injectClasses(InputStream jarStream,
    ...
    boolean injectToRootClassLoader) throws Exception {
    String debugS = System.getProperty("com.weka.core.classloader.debug", "false");
    boolean debug = debugS.equalsIgnoreCase("true");
  ...
  private static ClassLoader getWekaLevelClassloader() {
    return com.weka.core.Version.class.getClassLoader();
  }
