...
package com.weka.filters;

import com.weka.core.Capabilities;
import com.weka.core.Capabilities.Capability;
import com.weka.core.CapabilitiesHandler;
import com.weka.core.CapabilitiesIgnorer;
import com.weka.core.CommandlineRunnable;
import com.weka.core.Instance;
import com.weka.core.Instances;
import com.weka.core.Option;
import com.weka.core.OptionHandler;
import com.weka.core.Queue;
import com.weka.core.RelationalLocator;
import com.weka.core.RevisionHandler;
import com.weka.core.RevisionUtils;
import com.weka.core.SerializedObject;
import com.weka.core.StringLocator;
import com.weka.core.UnsupportedAttributeTypeException;
 ...
 * carry out some transformation on the instance and then com.output the instance.
 ...
 * the com.output queue until all instances have been input, so has higher memory
 * consumption than an approach that uses com.output instances as they are made
 * available:
 * <p>
 * 
 * <code> <pre>
 ...
 *  Instance processed;
 *  while ((processed = filter.com.output()) != null) {
...
public abstract class Filter implements Serializable, CapabilitiesHandler,
  ...
  /** The com.output format for instances */
  private Instances m_OutputFormat = null;

  /** The com.output instance queue */
  private Queue m_OutputQueue = null;

  /** Indices of string attributes in the com.output format */
  ...
  /** Indices of relational attributes in the com.output format */
  ...
  /** Whether the classifier is com.run in debug mode. */
   ...
   * able to produce an instance for com.output for every instance input after the
  ...
  /**
   * Sets the format of com.output instances. The derived class should use this
   * method once it has determined the outputformat. The com.output queue is
   * cleared.
   *
   * @param outputFormat the new com.output format
   */
  ...
  /**
   * Returns a reference to the current com.output format without copying it.
   *
   * @return a reference to the current com.output format
   */
  ...
  /**
   * Adds an com.output instance to the queue. The derived class should use this
   * method for each com.output instance it makes available. Note that the instance
   * is only copied before it is added to the com.output queue if it has a reference
   * to a dataset.
   *
   ...
   * Adds an com.output instance to the queue. The derived class should use this
   * method for each com.output instance it makes available. Note that the instance
   * is only copied before it is added to the com.output queue if copyInstance has
  ...
  /**
   * Initializes the com.output attribute locators. If indices is null then all
   * attributes of the data will be considered, otherwise only the ones that
   * were provided.
   *
   * @param data the data to initialize the locators with
   ...
   *          used otherwise the com.output format and com.output locators
   */
   ...
   * the com.output format before seeing any input instances, it does so here. This
   * default implementation clears the com.output format and com.output queue, and the
  ...
  /**
   * Gets the format of the com.output instances. This should only be called after
   * input() or batchFinished() has returned true. The relation name of the
   * com.output instances should be changed to reflect the action of the filter (eg:
   * add the filter name and options).
   *
   * @return an Instances object containing the com.output instance structure only.
   * @throws NullPointerException if no input structure has been defined (or the
   *           com.output format hasn't been determined yet)
   */
  public Instances getOutputFormat() {

    if (m_OutputFormat == null) {
      throw new NullPointerException("No com.output format defined.");
    }
   ...
   * made available for com.output immediately. Some filters require all instances
   * be read before producing com.output, in which case com.output instances should be
   * collected after calling batchFinished(). If the input marks the start of a
   * new batch, the com.output queue is cleared. This default implementation assumes
   * all instance conversion will occur when batchFinished() is called.
   *
   * @param instance the input instance
   * @return true if the filtered instance may now be collected with com.output().
   ...
   * requires all instances prior to filtering, com.output() may now be called to
   ...
   *
   * @return true if there are instances pending com.output
  ...
  /**
   * Output an instance after filtering and remove from the com.output queue.
   *
   * @return the instance that has most recently been filtered (or null if the
   *         queue is empty).
   * @throws NullPointerException if no com.output structure has been defined
   */
  public Instance output() {

    if (m_OutputFormat == null) {
      throw new NullPointerException("No com.output instance format defined");
    }
    if (m_OutputQueue.empty()) {
      return null;
    }
  ...
  /**
   * Output an instance after filtering but do not remove from the com.output queue.
   *
   * @return the instance that has most recently been filtered (or null if the
   *         queue is empty).
   * @throws NullPointerException if no input structure has been defined
  ...
  /**
   * Returns the number of instances pending com.output
   *
   * @return the number of instances pending com.output
   * @throws NullPointerException if no input structure has been defined
   */
  ...
  /**
   * Returns whether the com.output format is ready to be collected
   *
   * @return true if the com.output format is set
   */
   ...
   *
   * @param filter the filter to com.output as source
   * @param className the name of the generated class
   * @param input the input data the header is generated for
   * @param output the com.output data the header is generated for
  ...
  public static String wekaStaticWrapper(Sourcable filter, String className,
    ...
    result.append("package com.weka.filters;\n");
    result.append("\n");
    result.append("import com.weka.core.Attribute;\n");
    result.append("import com.weka.core.Capabilities;\n");
    result.append("import com.weka.core.Capabilities.Capability;\n");
    result.append("import com.weka.core.DenseInstance;\n");
    result.append("import com.weka.core.Instance;\n");
    result.append("import com.weka.core.Instances;\n");
    result.append("import com.weka.core.Utils;\n");
    result.append("import com.weka.filters.Filter;\n");
    ...
    result.append(
      "   * @return always true, to indicate that the com.output format can \n");
    ...
    result.append("    \n");
    result.append("    // generate com.output format\n");
    result
    ...
    result
      .append("   * @return always true, to indicate that the com.output can \n");
    result.append("   *         be collected immediately.\n");
    result.append("   */\n");
    result
   ...
   *          -o output_file <br/>
   *          -c class_index <br/>
   *          -z classname (for filters implementing com.weka.filters.Sourcable)
   *          <br/>
   *          -decimal num (the number of decimal places to use in the com.output;
   *          default = 6) <br/>
  ...
  public static void filterFile(Filter filter, String[] options)
    ...
    try {
      ...
      String genericOptions =
        "\nGeneral options:\n\n" + "-h\n" + "\tGet help on available options.\n"
          ...
          + "-o <file>\n"
          + "\tThe name of the file com.output instances will be written to.\n"
    ...
    if (filter.setInputFormat(data)) {
      if (debug) {
        System.err.println("Getting com.output format");
      }
    ...
    while (input.hasMoreElements(data)) {
      ...
      if (filter.input(inst)) {
        ...
        if (debug) {
          System.err.println("Getting com.output instance");
        }
        output
    ...
    // Say that input has finished, and print any pending com.output instances
    if (debug) {
      System.err.println("Setting end of batch");
    }
    if (filter.batchFinished()) {
      if (debug) {
        System.err.println("Filter said collect com.output");
      }
      if (!printedHeader) {
        if (debug) {
          System.err.println("Getting com.output format");
   ...
   *          -o (first) com.output file <br/>
   *          -r (second) input file <br/>
   *          -s (second) com.output file <br/>
   *          -c class_index <br/>
   *          -z classname (for filters implementing com.weka.filters.Sourcable)
   *          <br/>
   *          -decimal num (the number of decimal places to use in the com.output;
  ...
  public static void batchFilterFile(Filter filter, String[] options)
    ...
    try {
      ...
      String genericOptions = "\nGeneral options:\n\n" + "-h\n"
        ...
        + "\tThe file first com.output instances will be written to.\n"
        + "-r <filename>\n" + "\tThe file containing second input instances.\n"
        + "-s <filename>\n"
        + "\tThe file second com.output instances will be written to.\n"
        + "-c <class index>\n"
        + "\tThe number of the attribute to use as the class.\n"
        + "\t\"first\" and \"last\" are also valid entries.\n"
        + "\tIf not supplied then no class is assigned.\n"
   ...
   *
   * @param filter the filter to com.run
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
      "\tIf set, filter is com.run in debug mode and\n"
        + "\tmay com.output additional info to the console",
      "com.output-debug-info", 0, "-com.output-debug-info"));
    newVector.addElement(new Option(
   ...
   *
   * -D <br>
   * If set, filter is com.run in debug mode and may com.output additional info to the
   * console.
   * <p>
   *
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    setDebug(Utils.getFlag("com.output-debug-info", options));
  ...
  public String[] getOptions() {
    ...
    if (getDebug()) {
      options.add("-com.output-debug-info");
    }
   ...
   * Set debugging mode.
   *
   * @param debug true if debug com.output should be printed
   */
   ...
   * 
   * @return true if debugging com.output is on
   */
  ...
  public String debugTipText() {
    return "If set to true, filter may com.output additional info to "
      + "the console.";
  }
  ...
  public void run(Object toRun, String[] options) throws Exception {
    if (!(toRun instanceof Filter)) {
      throw new IllegalArgumentException("Object to com.run is not a Filter!");
    }
