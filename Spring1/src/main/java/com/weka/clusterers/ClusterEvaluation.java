 ...
 * specifies attribute values to be com.output with the predictions. Use '-p 0' for
 * none.
 * <p/>
 * 
 * -x num folds <br/>
 ...
 * clusterer that implemented the <code>com.weka.core.Drawable</code> interface.
 * <p/>
 * 
 * @author Mark Hall (mhall@cs.waikato.ac.nz)
 * @version $Revision$
 * @see com.weka.core.Drawable
 */
public class ClusterEvaluation implements Serializable, RevisionHandler {
   ...
   * @param testFileName the name of the test file for incremental testing, if
   *          "" or null then not used
   * @param outputModel true if the clustering model is to be com.output as well as
   *          the stats
   * 
   * @throws Exception if something goes wrong
   */
  ...
  public static String evaluateClusterer(Clusterer clusterer, String[] options)
    ...
    // Save the clusterer if an object com.output file is provided
    if (objectOutputFileName.length() != 0) {
