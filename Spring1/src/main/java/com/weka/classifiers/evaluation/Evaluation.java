...
package com.weka.classifiers.evaluation;

import com.weka.classifiers.AbstractClassifier;
import com.weka.classifiers.Classifier;
import com.weka.classifiers.ConditionalDensityEstimator;
import com.weka.classifiers.CostMatrix;
import com.weka.classifiers.IntervalEstimator;
import com.weka.classifiers.IterativeClassifier;
import com.weka.classifiers.Sourcable;
import com.weka.classifiers.UpdateableBatchProcessor;
import com.weka.classifiers.UpdateableClassifier;
import com.weka.classifiers.evaluation.output.prediction.AbstractOutput;
import com.weka.classifiers.evaluation.output.prediction.PlainText;
...
import com.weka.core.converters.ConverterUtils.DataSink;
...
import com.weka.core.xml.KOML;
import com.weka.core.xml.XMLOptions;
import com.weka.core.xml.XMLSerialization;
import com.weka.estimators.UnivariateKernelEstimator;
 ...
 *
 * -disable list <br/>
 * A comma separated list of metric names not to include in the com.output.
 * <p/>
 *
 * -l filename <br/>
 ...
 * <p/>
 *
 * -com.output-models-for-training-splits <br/>
 * Output models for training splits if cross-validation or percentage-split evaluation is used.
 * <p/>
 *
 * -do-not-com.output-per-class-statistics <br/>
 * Do not com.output statistics per class.
 * <p/>
 *
 * -k <br/>
 ...
 *
 * -classifications
 * "com.weka.classifiers.evaluation.com.output.prediction.AbstractOutput + options" <br/>
 * Uses the specified class for generating the classification com.output. E.g.:
 * com.weka.classifiers.evaluation.com.output.prediction.PlainText or :
 * com.weka.classifiers.evaluation.com.output.prediction.CSV
 *
 * -p range <br/>
...
public class Evaluation implements Summarizable, RevisionHandler, Serializable {
  ...
  /**
   * The list of metrics to display in the com.output
   */
  ...
  /**
   * Set a list of the names of metrics to have appear in the com.output. The
   * default is to display all built in metrics and plugin metrics that haven't
   * been globally disabled.
   *
   * @param display a list of metric names to have appear in the com.output
   */
  ...
  /**
   * Get a list of the names of metrics to have appear in the com.output The default
   * is to display all built in metrics and plugin metrics that haven't been
   * globally disabled.
   *
   * @return a list of metric names to have appear in the com.output
   */
  ...
  /**
   * Toggle the com.output of the metrics specified in the supplied list.
   *
   ...
   *                               expected to hold a
   *                               com.weka.classifiers.evaluation.com.output.prediction.AbstractOutput
   *                               object or a StringBuffer for model com.output
   * @throws Exception if a classifier could not be generated successfully or
   *                   the class is not defined
   */
  public void crossValidateModel(Classifier classifier, Instances data,
    ...
    // com.weka.classifiers.evaluation.com.output.prediction.AbstractOutput object
  ...
  public static String evaluateModel(Classifier classifier, String[] options) throws Exception {
    ...
    boolean outputModelsForTrainingSplits = Utils.getFlag("com.output-models-for-training-splits", options);
    boolean classStatistics = !Utils.getFlag("do-not-com.output-per-class-statistics", options);
    ...
    // We need to com.output help if something goes wrong with the option settings
    try {
      ...
      if (discardPredictions && (classifications.length() > 0 || classificationsOld.length() > 0)) {
        throw new IllegalArgumentException("Cannot both discard and com.output predictions!");
      }
      if (thresholdFile.length() > 0 && (classifications.length() > 0 || classificationsOld.length() > 0)) {
        throw new IllegalArgumentException("Cannot com.output predictions and also write threshold file!");
      }
      ...
      if ((thresholdFile.length() != 0) && discardPredictions) {
        throw new IllegalArgumentException("Can only com.output to threshold file when predictions are not discarded!");
      }
      if (outputModelsForTrainingSplits && (testFileName.length() > 0 ||
              ((splitPercentageString.length() == 0) && noCrossValidation))) {
        throw new IllegalArgumentException("Can only com.output models for training splits if cross-validation or " +
      ...
      // Need to check whether com.output of threshold file is possible if desired by user
      if ((thresholdFile.length() != 0) && !template.classAttribute().isNominal()) {
        throw new IllegalArgumentException("Can only com.output to threshold file when class attribute is nominal!");
      }

      // Need to check whether com.output of margins is possible if desired by user
      ...
      // Determine if predictions are to be com.output
      if (classifications.length() > 0) {
        classificationOutput = AbstractOutput.fromCommandline(classifications);
        if (classificationOutput == null) {
          throw new IllegalArgumentException("Failed to instantiate class for classification com.output: " + classifications);
        }
    ...
    if (trainFileName.length() > 0) {
      if (!noOutput || trainStatistics || printGraph || printSource || objectOutputFileName.length() > 0 ||
        ...
        if ((classifier instanceof UpdateableClassifier) && !forceBatchTraining) { // Build classifier incrementally
          ...
          Instances tempTrain = new DataSource(trainFileName).getDataSet(actualClassIndex);
          if (classifier instanceof com.weka.classifiers.misc.InputMappedClassifier) {
            Instances mappedClassifierDataset = ((com.weka.classifiers.misc.InputMappedClassifier) classifier)
            ...
            if (!mappedClassifierDataset.equalHeaders(tempTrain)) {
              for (int zz = 0; zz < tempTrain.numInstances(); zz++) {
                Instance mapped = ((com.weka.classifiers.misc.InputMappedClassifier) classifier)
    ...
    // If classifier is drawable com.output string describing graph
    if (printGraph) {
    ...
    // Stop here if no com.output of performance statistics or predictions is required and no threshold data is required
    ...
    if (classifier instanceof com.weka.classifiers.misc.InputMappedClassifier) {
      mappedClassifierHeader = ((com.weka.classifiers.misc.InputMappedClassifier) classifier)
              .getModelHeader(new Instances(template, 0));
    }

    // Do we just want to com.output predictions?
    if (classificationOutput != null) {

      // ===============================================
      // Code path for when predictions are to be com.output
      // ===============================================
  ...
  /**
   * Wraps a static classifier in enough source to test using the com.weka class
   * libraries.
   *
   * @param classifier a Sourcable Classifier
   * @param className the name to give to the source code class
   * @return the source for a static classifier that can be tested with com.weka
   *         libraries.
  ...
  public static String
    ...
    result.append("\n");
    result.append("package com.weka.classifiers;\n");
    result.append("\n");
    ...
    result.append("import com.weka.core.RevisionUtils;\n");
    result
      .append("import com.weka.classifiers.Classifier;\nimport com.weka.classifiers.AbstractClassifier;\n");
    result.append("\n");
  ...
  public String toClassDetailsString(String title) throws Exception {
    ...
    StringBuffer text =
      new StringBuffer(title + "\n                 "
        + "TP Rate___" + "FP Rate___"
        + "Precision___" + "Recall___"
        + "F-Measure___" + "MCC___"
        + "ROC Area___" + "PRC Area___");
    ...
    for (int i = 0; i < m_NumClasses; i++) {
      ...
      text.append("_____").append(m_ClassNames[i]).append('\n');
    }
  ...
  public double unweightedMacroFmeasure() {
    ...
    double[] trueClassCounts = trueClassCounts();
    double[] predictedClassCounts = predictedClassCounts();
    com.weka.experiment.Stats rr = new com.weka.experiment.Stats();
  ...
  protected static String makeOptionString(Classifier classifier,
    ...
    optionsText
      .append("\tComma separated list of metric names to toggle in the com.output.\n\t"
        + "All metrics are com.output by default with the exception of 'Coverage' and "
        + "'Region size'.\n\t");
    ...
    optionsText.append("-d <name of com.output file>\n");
    optionsText
      .append("\tSets model com.output file. In case the filename ends with '.xml',\n");
    optionsText
    ...
    optionsText.append("-com.output-models-for-training-splits\n");
    optionsText.append("\tOutput models for training splits if cross-validation or percentage-split evaluation is used.\n");
    optionsText.append("-do-not-com.output-per-class-statistics\n");
    optionsText.append("\tDo not com.output statistics for each class.\n");
    optionsText.append("-k\n");
    optionsText.append("\tOutputs information-theoretic statistics.\n");
    optionsText
      .append("-classifications \"com.weka.classifiers.evaluation.com.output.prediction.AbstractOutput + options\"\n");
    optionsText
      .append("\tUses the specified class for generating the classification com.output.\n");
