...
package com.weka.classifiers.trees.lmt;
 ...
 * (com.weka.classifiers.trees.lmt.LMT) and standalone logistic regression
 * (com.weka.classifiers.functions.SimpleLogistic).
 * 
 <!-- options-start -->
 * Valid options are:
 * <p/>
...
public class LogisticBase extends AbstractClassifier implements
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_fixedNumIterations > 0) {
      // com.run LogitBoost for fixed number of iterations
      ...
      // com.run LogitBoost using information criterion for stopping
      performBoostingInfCriterion();
    } else if (m_useCrossValidation) {
      // cross-validate number of LogitBoost iterations
      performBoostingCV();
    } else {
      // com.run LogitBoost with number of iterations that minimizes error on the
      // training set
      performBoosting();
    }
   ...
   * @param maxIterations the maximum number of LogitBoost iterations to com.run
   ...
   * 
   * @param numIterations the number of iterations to com.run
   * @throws Exception if something goes wrong
   */
