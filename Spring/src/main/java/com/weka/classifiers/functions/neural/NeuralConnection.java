...
package com.weka.classifiers.functions.neural;
...
public abstract class NeuralConnection
  ...
  /** The com.output value for this unit, NaN if not calculated. */
  ...
  /**
   * Call this to reset the unit for another com.run.
  ...
  /**
   * Use this to get easy access to the com.output numbers.
   * It is not advised to change the entries in this list
   * (use the connecting and disconnecting functions to do that)
   * @return The outputs list.
   */
  ...
  /**
   * Call this function to draw the nodes com.output connections.
   * @param g The graphics context.
   * @param w The width of the drawing area.
   * @param h The height of the drawing area.
   */
  ...
  /** 
   * This will connect the specified unit to be an com.output to this unit.
   * @param o The unit.
   * @param n It's connection number for this connection.
   * @return True if the connection was made, false otherwise.
   */
  ...
  /**
   * Allocates more space for com.output connection information
  ...
  /**
   * This will disconnect the com.output with the specific connection number
   * From this node (only on this end however).
  ...
  /**
   * @return The number of com.output connections.
   */
  ...
  public static boolean connect(NeuralConnection s, NeuralConnection t) {
    ...
    if ((s.getType() & PURE_OUTPUT) == PURE_OUTPUT) {
      return false;   //source is an com.output node
    }
    ...
    if ((t.getType() & PURE_OUTPUT) == PURE_OUTPUT && t.getNumInputs() > 0) {
      return false; //more than 1 node is trying to feed a particular com.output
    }

    if ((t.getType() & PURE_OUTPUT) == PURE_OUTPUT &&
	(s.getType() & OUTPUT) == OUTPUT) {
      return false; //an com.output node already feeding out a final answer
    }
