...
package com.weka.classifiers.pmml.consumer;
...
import com.weka.core.pmml.DerivedFieldMetaInfo;
import com.weka.core.pmml.FieldMetaInfo;
import com.weka.core.pmml.MiningSchema;
import com.weka.core.pmml.NormContinuous;
import com.weka.core.pmml.TargetMetaInfo;
...
public class NeuralNetwork extends PMMLClassifier {
  ...
  class NeuralLayer implements Serializable {
    ...
    /** Stores the com.output of this layer (for given inputs) */
    ...
    /**
     * Compute the com.output values for this layer.
     * 
     * @param incoming the incoming values
     * @return the com.output values for this layer
...
//          System.err.println("Normalizing ID " + m_layerNeurons[i].getID() + " " + val);
  ...
  static class Neuron implements Serializable {
    ...
    /**
     * Compute the com.output of this Neuron.
     * 
     * @param incoming a Map of input values. The keys are the IDs
     * of incoming connections (either neural inputs or neurons) and
     * the values are the com.output values of the neural input/neuron in
     * question.
     * 
     * @return the com.output of this neuron
  ...
  static class NeuralOutputs implements Serializable {
     ...
     * being predicted by each com.output neuron
     */
    ...
    protected NeuralOutputs(Element outputs, MiningSchema miningSchema) throws Exception {
      ...
      for (int i = 0; i < outputL.getLength(); i++) {
        ...
        if (outputN.getNodeType() == Node.ELEMENT_NODE) {
          ...
          // get the ID for this com.output neuron
          ...
          if (m_classAttribute.isNumeric()) {
            ...
            // that this com.output neuron is associated with
            ...
            if (discL.getLength() != 1) {
              throw new Exception("[NeuralOutputs] Should be only one norm discrete element "
                  + "per derived field/neural com.output for a nominal class!");
            }
    ...
    /**
     * Compute the com.output. Either a probability distribution or a single
     * value (regression).
     * 
     * @param incoming the values from the last hidden layer
     * @param preds the array to fill with predicted values
     * @throws Exception if there is a problem computing the com.output
     */
    protected void getOuput(HashMap<String, Double> incoming, double[] preds) throws Exception {
      ...
      for (int i = 0; i < m_outputNeurons.length; i++) {
        ...
        if (neuronOut == null) {
          throw new Exception("[NeuralOutputs] Unable to find com.output neuron "
              + m_outputNeurons[i] + " in the incoming HashMap!!");
        }
        if (m_classAttribute.isNumeric()) {
          // will be only one com.output neuron anyway
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    if (hasMissing) {
      ...
      // now compute the com.output of each layer
