...
import com.weka.core.pmml.MappingInfo;
...
public abstract class PMMLClassifier extends AbstractClassifier
  ...
  /**
   * Signal that a scoring com.run has been completed. Resets
   * the initialized state to false so that a subsequent
   * scoring com.run will trigger the mapping of the mining
   * schema to incoming instances. If not called after a
   * scoring com.run, then the classifier will assume that
