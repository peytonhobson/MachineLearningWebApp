...
public class FlowRunner implements RevisionHandler {
  ...
  /** com.run each Startable bean sequentially? (default in parallel) */
  protected boolean m_startSequentially = false;

  public static class SimpleLogger implements com.weka.gui.Logger {
  ...
  public void load(String fileName) throws Exception {
    if (!fileName.endsWith(".kf") && !fileName.endsWith(".kfml")) {
      throw new Exception(
        "Can only load and com.run binary or xml serialized KnowledgeFlows "
          + "(*.kf | *.kfml)");
    }
  ...
  /**
   * Set the vector holding the flows(s) to com.run
   * 
   * @param beans the Vector holding the flows to com.run
   */
