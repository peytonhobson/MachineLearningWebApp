...
import com.weka.associations.AssociationRules;
import com.weka.associations.Associator;
import com.weka.associations.FPGrowth;
...
import com.weka.gui.visualize.plugins.AssociationRuleVisualizePlugin;
...
/**
 * This panel allows the user to select, configure, and com.run a scheme that learns
 * associations.
 * 
 * @author Eibe Frank (eibe@cs.waikato.ac.nz)
 * @version $Revision$
 */
@PerspectiveInfo(ID = "com.weka.gui.explorer.associationspanel",
...
public class AssociationsPanel extends AbstractPerspective implements
  ...
  /** The com.output area for associations */
  ...
  /**
   * Whether to store any graph or xml rules com.output in the history list
   */
  protected JCheckBox m_storeOutput = new JCheckBox(
    "Store com.output for visualization");
  ...
  public AssociationsPanel() {
    ...
    p3.setBorder(BorderFactory.createTitledBorder("Associator com.output"));
   ...
   * no associator already running. The associator com.output is sent to the results
   * history panel.
   */
    ...
    if (m_RunThread == null) {
      ...
      m_RunThread = new Thread() {
        ...
        public void run() {
          ...
          if (cname.startsWith("com.weka.associations.")) {
            name += cname.substring("com.weka.associations.".length());
          } else {
            name += cname;
          }
          ...
          try {
            ...
            if (m_storeOutput.isSelected()) {
              ...
              if (associator instanceof com.weka.associations.AssociationRulesProducer) {
                // xmlRules = null;
                rulesList = null;
                try {
                  // xmlRules =
                  // ((com.weka.associations.XMLRulesProducer)associator).xmlRules();
                  rulesList =
                    ((com.weka.associations.AssociationRulesProducer) associator)
                      .getAssociationRules();
                } catch (Exception ex) {
                }
              }
  ...
  /**
   * Save the currently selected associator com.output to a file.
   * 
  ...
  protected static final class AssociationsPanelDefaults extends Defaults {
    public static final String ID = "com.weka.gui.explorer.associationspanel";
