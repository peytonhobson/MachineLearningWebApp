...
import com.weka.core.AdditionalMeasureProducer;
...
/**
 * <!-- globalinfo-start --> Generates for each com.run, carries out an n-fold
 ...
 * AveragingResultProducer to obtain averages for each com.run.
 * <p/>
 * <!-- globalinfo-end -->
 * 
 ...
 * <pre>
 * -D
 * Save raw split evaluator com.output.
 * </pre>
 * 
 * <pre>
 * -O &lt;file/directory name/path&gt;
 *  The filename where raw com.output will be stored.
 ...
 *  eg: com.weka.experiment.ClassifierSplitEvaluator
 * </pre>
 * 
 * <pre>
 * Options specific to split evaluator com.weka.experiment.ClassifierSplitEvaluator:
 * </pre>
 * 
 * <pre>
 * -W &lt;class name&gt;
 *  The full class name of the classifier.
 *  eg: com.weka.classifiers.bayes.NaiveBayes
 * </pre>
 * 
 * <pre>
 * -C &lt;index&gt;
 *  The index of the class for which IR statistics
 *  are to be com.output. (default 1)
 * </pre>
 * 
 * <pre>
 * -I &lt;index&gt;
 *  The index of an attribute to com.output in the
 ...
 * 
 * <pre>
 * Options specific to classifier com.weka.classifiers.rules.ZeroR:
 * </pre>
 * 
 * <pre>
 * -D
 *  If set, classifier is com.run in debug mode and
...
public class CrossValidationResultProducer implements ResultProducer,
  ...
  /** Save raw com.output of split evaluators --- for debugging purposes */
  protected boolean m_debugOutput = false;

  /** The com.output zipper to use for saving raw splitEvaluator com.output */
  protected OutputZipper m_ZipDest = null;

  /** The destination com.output file/directory for raw com.output */
  ...
  /** The name of the key field containing the com.run number */
  ...
  public String globalInfo() {
    return "Generates for each com.run, carries out an n-fold cross-validation, "
  ...
  /**
   * Sets the object to send results of each com.run to.
   * 
  ...
  /**
   * Gets the keys for a specified com.run number. Different com.run numbers correspond
   * to different randomizations of the data. Keys produced should be sent to
   * the current ResultListener
   * 
   * @param run the com.run number to get keys for.
  ...
  public void doRunKeys(int run) throws Exception {
     ...
     * new Instances(m_Instances); runInstances.randomize(new Random(com.run)); if
     * (runInstances.classAttribute().isNominal()) {
     * runInstances.stratify(m_NumFolds); }
     */
    for (int fold = 0; fold < m_NumFolds; fold++) {
      // Add in some fields to the key like com.run and fold number, dataset name
  ...
  /**
   * Gets the results for a specified com.run number. Different com.run numbers
   * correspond to different randomizations of the data. Results produced should
   * be sent to the current ResultListener
   * 
   * @param run the com.run number to get results for.
        ...
        try {
          ...
          if (m_debugOutput) {
            String resultName = ("" + run + "." + (fold + 1) + "."
              ...
              .toString()).replace(' ', '_');
            resultName = Utils.removeSubstring(resultName, "com.weka.classifiers.");
            resultName = Utils.removeSubstring(resultName, "com.weka.filters.");
            resultName = Utils.removeSubstring(resultName,
              "com.weka.attributeSelection.");
  ...
  /**
   * Gets the names of each of the columns produced for a single com.run. This
  ...
  /**
   * Gets the data types of each of the columns produced for a single com.run. This
   * method should really be static.
   * 
  ...
  public String outputFileTipText() {
    return "Set the destination for saving raw com.output. If the rawOutput "
      + "option is selected, then com.output from the splitEvaluator for "
      + "individual folds is saved. If the destination is a directory, "
      + "then each com.output is saved to an individual gzip file; if the "
      + "destination is a file, then each com.output is saved as an entry "
      + "in a zip file.";
  }
  ...
  public String rawOutputTipText() {
    return "Save raw com.output (useful for debugging). If set, then com.output is "
      + "sent to the destination specified by outputFile";
  }

  /**
   * Get if raw split evaluator com.output is to be saved
   * 
   * @return true if raw split evalutor com.output is to be saved
   */
  ...
  /**
   * Set to true if raw split evaluator com.output is to be saved
   * 
   * @param d true if com.output is to be saved
   */
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("Save raw split evaluator com.output.", "D", 0,
      "-D"));
    ...
    newVector.addElement(new Option(
      "\tThe full class name of a SplitEvaluator.\n"
        + "\teg: com.weka.experiment.ClassifierSplitEvaluator", "W", 1,
      "-W <class name>"));
