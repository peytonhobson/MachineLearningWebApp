 ...
 * <br>
 * 
 * java com.weka.Run NaiveBayes <br>
 * <br>
 * 
 * will prompt the user to choose among
 * com.weka.classifiers.bayes.ComplementNaiveBayes,
 * com.weka.classifiers.bayes.NaiveBayes,
 * com.weka.classifiers.bayes.NaiveBayesMultinomial,
 * com.weka.classifiers.bayes.NaiveBayesMultinomialUpdateable,
 * com.weka.classifiers.bayes.NaiveBayesSimple,
 * com.weka.classifiers.bayes.NaiveBayesUpdateable
 * 
...
public class Run {
  ...
  public static List<String> findSchemeMatch(Class<?> classType,
    ...
    com.weka.core.ClassDiscovery.clearCache();
    ArrayList<String> matches = com.weka.core.ClassDiscovery.find(schemeToFind);
      ...
      if (matches.get(i).endsWith(schemeToFind) || matchAnywhere) {
        try {
          ...
          if (classType == null
            ...
            if (notJustRunnables
              || scheme instanceof com.weka.classifiers.Classifier
              || scheme instanceof com.weka.clusterers.Clusterer
              || scheme instanceof com.weka.associations.Associator
              || scheme instanceof com.weka.attributeSelection.ASEvaluation
              || scheme instanceof com.weka.filters.Filter
              || scheme instanceof com.weka.core.converters.AbstractFileLoader
              || scheme instanceof com.weka.core.converters.AbstractFileSaver
              || scheme instanceof com.weka.datagenerators.DataGenerator
              || scheme instanceof com.weka.core.CommandlineRunnable) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (Utils.getFlag("list-packages", args)) {
        com.weka.core.WekaPackageManager.loadPackages(true, true, false);
        return;
      }
      ...
      if (!noLoad) {
        com.weka.core.WekaPackageManager.loadPackages(false, true, false);
      }
      ...
      if (!noScan) {
        ...
        if (prunedMatches.size() == 0) {
          ...
          boolean done = false;
          while (!done) {
            System.out.println("Select a scheme to com.run, or <return> to exit:");
            for (int i = 0; i < prunedMatches.size(); i++) {
              System.out.println("\t" + (i + 1) + ") " + prunedMatches.get(i));
            }
            System.out.print("\nEnter a number > ");
      ...
      if (scheme instanceof com.weka.core.CommandlineRunnable) {
        types.add(SchemeType.COMMANDLINE);
      } else {
        if (scheme instanceof com.weka.classifiers.Classifier) {
          types.add(SchemeType.CLASSIFIER);
        }
        if (scheme instanceof com.weka.clusterers.Clusterer) {
          types.add(SchemeType.CLUSTERER);
        }
        if (scheme instanceof com.weka.associations.Associator) {
          types.add(SchemeType.ASSOCIATOR);
        }
        if (scheme instanceof com.weka.attributeSelection.ASEvaluation) {
          types.add(SchemeType.ATTRIBUTE_SELECTION);
        }
        if (scheme instanceof com.weka.filters.Filter) {
          types.add(SchemeType.FILTER);
        }
        if (scheme instanceof com.weka.core.converters.AbstractFileLoader) {
          types.add(SchemeType.LOADER);
        }
        if (scheme instanceof com.weka.core.converters.AbstractFileSaver) {
          types.add(SchemeType.SAVER);
        }
        if (scheme instanceof com.weka.datagenerators.DataGenerator) {
      ...
      if (selectedType == SchemeType.CLASSIFIER) {
        com.weka.classifiers.AbstractClassifier.runClassifier(
          (com.weka.classifiers.Classifier) scheme, options);
      } else if (selectedType == SchemeType.CLUSTERER) {
        com.weka.clusterers.AbstractClusterer.runClusterer(
          (com.weka.clusterers.Clusterer) scheme, options);
      } else if (selectedType == SchemeType.ATTRIBUTE_SELECTION) {
        com.weka.attributeSelection.ASEvaluation.runEvaluator(
          (com.weka.attributeSelection.ASEvaluation) scheme, options);
      } else if (selectedType == SchemeType.ASSOCIATOR) {
        com.weka.associations.AbstractAssociator.runAssociator(
          (com.weka.associations.Associator) scheme, options);
      } else if (selectedType == SchemeType.FILTER) {
        com.weka.filters.Filter.runFilter((com.weka.filters.Filter) scheme, options);
      } else if (selectedType == SchemeType.LOADER) {
        com.weka.core.converters.AbstractFileLoader.runFileLoader(
          (com.weka.core.converters.AbstractFileLoader) scheme, options);
      } else if (selectedType == SchemeType.SAVER) {
        com.weka.core.converters.AbstractFileSaver.runFileSaver(
          (com.weka.core.converters.AbstractFileSaver) scheme, options);
      } else if (selectedType == SchemeType.DATAGENERATOR) {
        com.weka.datagenerators.DataGenerator.runDataGenerator(
          (com.weka.datagenerators.DataGenerator) scheme, options);
      } else if (selectedType == SchemeType.COMMANDLINE) {
        ((com.weka.core.CommandlineRunnable) scheme).run(scheme, options);
      }
    } catch (Exception e) {
