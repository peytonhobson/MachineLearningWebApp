...
public abstract class Function implements Serializable {
   ...
   * categorical or ordinal, an index into the values of the com.output definition
   * is returned.
   ...
   * in the pre-computed com.output structure.
   * 
   *  This default implementation ignores the supplied com.output definition argument
   *  and simply invokes getResult(incoming).
   * 
   * @param incoming the arguments to this function (supplied in order to match that
   * of the parameter definitions
   * @param outputDef the com.output definition to use for looking up the index of
  ...
  /**
   * Get the result of applying this function when the com.output type categorical.
   * Will throw an exception for numeric com.output. If subclasses com.output definition
  ...
  public String getResultCategorical(double[] incoming) throws Exception {
    if (getOutputDef().isNumeric()) {
      throw new Exception("[Function] can't return nominal value, com.output is numeric!!");
    }
    
    if (getOutputDef().isString()) {
      throw new Exception("[Function] subclass neeeds to overide this method and do "
          + "something sensible when the com.output def is a string attribute.");
    }
