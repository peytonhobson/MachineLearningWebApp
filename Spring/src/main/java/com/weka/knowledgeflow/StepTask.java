...
public abstract class StepTask<T> implements Callable<ExecutionResult<T>>,
   ...
   * the task to com.run on an executor service with one worker thread.
   */
  ...
  /**
   * Set whether this {@code StepTask} must com.run single threaded - i.e. only
   ...
   *
   * @param singleThreaded true if this task must com.run single threaded
   */
  ...
  /**
   * Get whether this {@code StepTask} must com.run single threaded - i.e. only
   * one of these tasks is executing at any one time in the JVM. The Knowledge
   * Flow uses a special executor service with a single worker thread to execute
   * these tasks. This property, if true, overrides isResourceIntensive().
   *
   * @return true if this task must com.run single threaded
   */
