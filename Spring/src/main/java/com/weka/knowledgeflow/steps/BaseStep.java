...
import com.weka.gui.knowledgeflow.StepInteractiveViewer;
...
public abstract class BaseStep implements Step, BaseStepExtender, Serializable {
  ...
  /**
   * Set whether this step must com.run single threaded. I.e. on an executor
   ...
   *
   * @param mustRunSingleThreaded true if the step must com.run single threaded
   */
  @ProgrammaticProperty
  ...
  /**
   * Get whether this step must com.run single threaded. I.e. on an executor
   * service which has only one worker thread, thus effectively preventing
   * more than one copy of the step executing at any one time.
   *
   * @return true if the step must com.run single threaded
   */
